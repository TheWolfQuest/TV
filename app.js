...
function loadState(){
  ...
  try{
    ...
    if(s&&Array.isArray(s.catalog)){
      ...
      s.catalog.forEach(show=>{show.type=inferType(show)});
      s.schemaVersion=3;
      localStorage.setItem(STORAGE_KEY,JSON.stringify(s));
      return s;
    }
  }catch{}
  seed.forEach(show=>{show.type=inferType(show)});
...
$('addShow').onclick=()=>{$('addForm').reset();$('addDialog').showModal()};$('cancelAdd').onclick=()=>$('addDialog').close();$('addForm').onsubmit=e=>{e.preventDefault();const id=`user-${Date.now()}`,base={id,title:$('addTitle').value.trim(),network:$('addNetwork').value.trim(),type:$('addType').value,when:$('addWhen').value.trim(),status:$('addStatus').value,notes:''};state.catalog.push(base);const r=record(id);r.status=base.status;r.notes=$('addNotes').value.trim();save();populateServices();$('addDialog').close();renderTable();toast('Show added')};
