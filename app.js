...
const STANDARD_SERVICES=['Unknown','Netflix','Prime Video','Hulu','Apple TV+','Disney+','Max','Peacock','Paramount+','MGM+','AMC+','Starz','Showtime','BritBox','Acorn TV','PBS','YouTube','Tubi','Pluto TV','The Roku Channel','Freevee','Hallmark+','Other'];
const SERVICE_ALIASES={
  '':'Unknown','unknown':'Unknown','n/a':'Unknown','na':'Unknown','tbd':'Unknown',
  'netflix':'Netflix','hulu':'Hulu','peacock':'Peacock','max':'Max','hbo max':'Max','hbo':'Max',
  'prime':'Prime Video','amazon prime':'Prime Video','amzn prime':'Prime Video','amaz prime':'Prime Video','prime video':'Prime Video',
  'apple':'Apple TV+','apple tv':'Apple TV+','apple tv+':'Apple TV+',
  'disney':'Disney+','disney+':'Disney+',
  'paramount':'Paramount+','paramount+':'Paramount+','cbs/paramount':'Paramount+','cbs/paramount+':'Paramount+',
  'mgm':'MGM+','mgm+':'MGM+','amc':'AMC+','amc+':'AMC+','starz':'Starz','showtime':'Showtime',
  'britbox':'BritBox','acorn':'Acorn TV','acorn tv':'Acorn TV','pbs':'PBS','youtube':'YouTube','tubi':'Tubi',
  'pluto':'Pluto TV','pluto tv':'Pluto TV','roku':'The Roku Channel','roku channel':'The Roku Channel','the roku channel':'The Roku Channel',
  'freevee':'Freevee','hallmark':'Hallmark+','hallmark+':'Hallmark+','other':'Other'
};
function normalizeService(value){const raw=String(value??'').trim();if(!raw)return 'Unknown';return SERVICE_ALIASES[raw.toLowerCase()]||raw}
function serviceOptions(current){const custom=[...new Set(state.catalog.map(x=>normalizeService(x.network)).filter(x=>x&&!STANDARD_SERVICES.includes(x)))].sort();const options=[...STANDARD_SERVICES,...custom];if(current&&!options.includes(current))options.push(current);return options}
const $=id=>document.getElementById(id);
let state=loadState(),currentPage='library',sort={key:'title',dir:1};
function inferType(show){if(show&&['Series','Movie'].includes(show.type))return show.type;const text=`${show?.title||''} ${show?.network||''} ${show?.originalNetwork||''}`.toLowerCase();return /\bmovie\b|\bfilm\b|\btheater\b/.test(text)?'Movie':'Series'}
function loadState(){
  ...
  try{
    ...
    if(s&&Array.isArray(s.catalog)){
      ...
      s.catalog.forEach(show=>{show.type=inferType(show);show.network=normalizeService(show.network)});
      s.schemaVersion=3;
      localStorage.setItem(STORAGE_KEY,JSON.stringify(s));
      return s;
    }
  }catch{}
  seed.forEach(show=>{show.type=inferType(show);show.network=normalizeService(show.network)});
  return{schemaVersion:3,catalog:seed,knownSeedIds:seed.map(x=>x.id),deletedIds:[],profiles:[...DEFAULT_PROFILES],active:'Keith',personal:{},timeline:{},feedback:[],theme:'dark'}
}
...
<td><select class="cell-select service-select" data-id="${x.id}" data-field="network">${serviceOptions(x.network).map(v=>`<option${v===x.network?' selected':''}>${esc(v)}</option>`).join('')}</select></td>
<td><select class="cell-select type-select" data-id="${x.id}" data-field="type">${['Series','Movie'].map(v=>`<option${v===x.type?' selected':''}>${v}</option>`).join('')}</select></td>
...
function commitInline(el){const id=el.dataset.id,field=el.dataset.field,base=state.catalog.find(x=>x.id===id),r=record(id);if(!base)return;if(field==='title'||field==='when')base[field]=el.value.trim();else if(field==='network')base.network=normalizeService(el.value);else if(field==='notes')r.notes=el.value.trim();else if(field==='watched'){const p=parseProgress(el.value);if(!el.value.trim()){r.season=0;r.episode=0}else if(p){r.season=p.season;r.episode=p.episode;r.status='Watching';addTimeline(id,`Set watched to ${progress(r.season,r.episode)}`)}else{toast('Use S2 E4 format');el.value=progress(r.season,r.episode);return}}save();if(field==='network')populateServices();toast('Saved')}
function bindInline(){document.querySelectorAll('.cell-edit').forEach(el=>{el.addEventListener('change',()=>commitInline(el));el.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();commitInline(el);el.blur()}if(e.key==='Escape'){renderTable()}})});document.querySelectorAll('.cell-select').forEach(el=>el.onchange=()=>{if(el.dataset.field==='type'){const base=state.catalog.find(x=>x.id===el.dataset.id);if(base)base.type=el.value}else{record(el.dataset.id).status=el.value;addTimeline(el.dataset.id,`Status changed to ${el.value}`)}save();toast('Saved')});document.querySelectorAll('[data-fav]').forEach(el=>el.onclick=()=>{const r=record(el.dataset.fav);r.favorite=!r.favorite;save();renderTable()});document.querySelectorAll('[data-rating]').forEach(el=>el.onclick=()=>{const r=record(el.dataset.rating);r.rating=String(((+r.rating||0)+1)%6||'');save();renderTable()});document.querySelectorAll('[data-delete]').forEach(el=>el.onclick=()=>deleteShow(el.dataset.delete))}
...
function populateServices(){const selected=$('service').value;$('service').innerHTML='<option value="">All services</option>'+[...new Set(state.catalog.map(x=>normalizeService(x.network)).filter(Boolean))].sort().map(x=>`<option>${esc(x)}</option>`).join('');$('service').value=selected}
...
document.querySelectorAll('nav button').forEach(e=>e.onclick=()=>setPage(e.dataset.page));document.querySelectorAll('[data-page-link]').forEach(e=>e.onclick=()=>setPage(e.dataset.pageLink));document.querySelectorAll('[data-open-library]').forEach(e=>e.onclick=()=>{$('quick').value=e.dataset.openLibrary==='all'?'all':e.dataset.openLibrary;setPage('library')});document.querySelectorAll('.cards button[data-filter]').forEach(e=>e.onclick=()=>{$('type').value='';$('quick').value=e.dataset.filter;setPage('library')});document.querySelectorAll('.cards button[data-type-card]').forEach(e=>e.onclick=()=>{$('quick').value='all';$('type').value=e.dataset.typeCard;setPage('library')});
$('profile').onchange=()=>{state.active=$('profile').value;save();renderProfiles();setPage(currentPage)};$('theme').onclick=()=>{state.theme=state.theme==='light'?'dark':'light';document.body.classList.toggle('light',state.theme==='light');save()};['search','service','status','type','quick'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',renderTable));$('clearSearch').onclick=()=>{$('search').value='';renderTable();$('search').focus()};document.querySelectorAll('th[data-sort]').forEach(th=>th.onclick=()=>{sort=sort.key===th.dataset.sort?{key:sort.key,dir:-sort.dir}:{key:th.dataset.sort,dir:1};renderTable()});
$('addShow').onclick=()=>{$('addForm').reset();$('addDialog').showModal()};$('cancelAdd').onclick=()=>$('addDialog').close();$('addForm').onsubmit=e=>{e.preventDefault();const id=`user-${Date.now()}`,base={id,title:$('addTitle').value.trim(),network:normalizeService($('addNetwork').value),type:$('addType').value,when:$('addWhen').value.trim(),status:$('addStatus').value,notes:''};state.catalog.push(base);const r=record(id);r.status=base.status;r.notes=$('addNotes').value.trim();save();populateServices();$('addDialog').close();renderTable();toast('Show added')};
